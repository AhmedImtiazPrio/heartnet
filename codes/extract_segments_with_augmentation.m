...
file_name=[];
states=[];
filter_path='/media/taufiq/Data/heart_sound/feature/order37filtersforaugmentation.mat';
...
%% Pre-processing (resample + bandpass + spike removal)
    ...
    PCG_augmented = filtfilt(b{folder_idx+1},1,PCG_resampled);  % Apply augmentation filter
...
%% Dividing signals into filter banks
    clear PCG
    PCG(:,1) = filtfilt(b1,1,PCG_augmented);
    PCG(:,2) = filtfilt(b2,1,PCG_augmented);
    PCG(:,3) = filtfilt(b3,1,PCG_augmented);
    PCG(:,4) = filtfilt(b4,1,PCG_augmented);
