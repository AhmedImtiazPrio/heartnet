...
%% Dividing signals into filter banks
    ...
    for row=1:ncc
%         for fb=1:nfb
%             if row == ncc % for the last cardiac cycle take all of the signal
                tmp = PCG(idx_states(row,1):end);
                if length(tmp)>=nsamp 
                    tmp=tmp(1:nsamp);
                else
                    N = nsamp-length(tmp);
                    tmp = repmat(tmp,1,ceil(N/length(tmp))+1);
                    tmp = tmp(1:nsamp);
                end               
%             else
%                 tmp = PCG(idx_states(row,1):idx_states(row,1)+nsamp-1,fb);
%             end
%             length(tmp)
            x(row,:) = tmp;
%         end
...
%% Save Data
    sname=[savedir 'training-' 'a'+folder_idx '_cont_noFIR' '.mat'];
